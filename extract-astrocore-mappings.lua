...
for line in mappings_section:gmatch "[^\r\n]+" do
  ...
  if not key and not cmd and not desc then
    ...
    if key and desc then cmd = "function" end
  end
  if key then
    if not mappings[current_mode] then mappings[current_mode] = {} end
...
local function organize_by_prefix(mode_mappings)
  ...
  for _, mapping in ipairs(mode_mappings) do
    ...
    if key:match "^<Leader>" then
      ...
      if prefix then
        if not organized[prefix] then organized[prefix] = {} end
      ...
      -- Non-leader mappings
      if not organized["_other"] then organized["_other"] = {} end
